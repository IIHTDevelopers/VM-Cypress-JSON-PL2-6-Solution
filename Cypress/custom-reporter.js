...
class CypressCustomReporter {
    constructor() {
        ...
        // Load custom data if available
            const customDataPath = '../../custom.ih';
            if (fs.existsSync(customDataPath)) {
                try {
                    const data = fs.readFileSync(customDataPath, 'utf8');
                    this.customData = data;
                } catch (err) {
                    console.error('Error reading custom data:', err.message);
                }
            } else {
                console.warn(`Custom data file not found at ${customDataPath}. Proceeding without it.`);
                this.customData = '';
            }
